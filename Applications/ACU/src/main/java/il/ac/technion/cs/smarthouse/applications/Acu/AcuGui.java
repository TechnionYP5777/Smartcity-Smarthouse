...
public class AcuGui extends SmarthouseApplication implements Simulatable {
    static Simulation simulation = new Simulation(30, // summer sucks
                    "Roy's Man Cave", "Ron's Home Cinema", "Yarden's AI lab", "Inbal's dogs kennel");
    ...
    public void onLoad() throws Exception {
        ...
        wantedTemp.addOnDataChangedListener(desiredT -> desiredT.getDataAsOptional()
                        .ifPresent(t -> sensor.instruct(t + 0.2 + "", setTempPath)));
        ...
        getAppBuilder().getConfigurationsRegionBuilder().addIntegerInputField("Desired Temperature:", wantedTemp)
                        .addSensorAliasSelectionField("The Controlled location:", sensor, (oldA, newA) -> {
                            ...
                            wantedTemp.getDataAsOptional().ifPresent(
                                            t -> super.<SensorsService>getService(ServiceType.SENSORS_SERVICE)
        ...
        simulation.getAliases().forEach(alias -> {
            ...
            getAppBuilder().getWidgetsRegionBuilder().addWidget(WidgetType.PROGRESS_LINE_GRAPH,
                            new InfoCollector().addInfoEntry(getTempPath, "temperature").setTitle(alias), currSensor,
                            acu -> {
    ...
    public class ACUSensor extends SensorData {
        @SystemPath("ACUnit.current.state") private boolean isOn;
        @SystemPath("ACUnit.current.temperature") private int temp;
