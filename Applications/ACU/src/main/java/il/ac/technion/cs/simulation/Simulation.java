...
/**
 * The simulation randomize temperatures to the given rooms, and get closer to
 * the default given temperature as time progresses. If the room sensor receive
 * an instruction to turn on an ACU, the temperature will increase/decrease
...
public class Simulation {
    ...
    public Simulation(Integer defaultTemp, String... aliases) {
        ...
        Stream.of(aliases).map(key -> key + " ACU sensor").forEach(alias -> {
            GenericSensor s = builder.setAlias(alias).setMessageSupplier(() -> sensors.get(alias).getCurrentState())
                            .setInstructionHandler((p, v) -> sensors.get(alias).processInstruction(p, v)).build();
            final String id = simulator.addSensor(s);
            sensors.put(alias, new AcuState(id, naturalRoomTemperature));
        });
    }

    public static String getPath(PathType t, String suffix) {
        return String.join(".", commname, PathType.INFO_SENDING.equals(t) ? obsersBase : instsBase, suffix);
    }
