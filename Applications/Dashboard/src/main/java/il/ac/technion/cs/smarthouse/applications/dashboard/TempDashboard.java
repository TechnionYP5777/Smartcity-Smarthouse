...
public class TempDashboard extends Application {
	...
	@Override
	public void init() {
		textTile = TileBuilder.create().prefSize(TILE_SIZE, TILE_SIZE).skinType(SkinType.TEXT).title("Text Tile")
				.text("Whatever text").description("Click Meeeee :>").textVisible(true).build();
	...
	public void updateTile(String type) {
		switch (type) {
		case "Clock":
			Tile clockTile = TileBuilder.create().prefSize(150, 150).skinType(SkinType.CLOCK).title("Clock Tile")
