...
public class ConfigController extends SystemGuiController {
    private class ButtonCell extends TableCell<NamedPath, String> {
        ...
        protected void updateItem(final String item, final boolean empty) {
            ...
            setGraphic(empty ? null : cellButton); // Display button if the row
                                                   // is not empty
        }
    }
    ...
    private void addWidgetTimer(final BasicWidget widget) {
        ...
        where.add(new AnimationTimer() {
            ...
            final Boolean isGraph = key
                            .equals(new GraphWidget(WidgetType.AREA_GRAPH, widget.getTileSize(), widget.getInitalInfo())
                                            .getTitle());
            final Boolean isList = key
                            .equals(new ListWidget(WidgetType.BAR_CHART, widget.getTileSize(), widget.getInitalInfo())
                                            .getTitle());
    ...
    private List<String> getAvailablePaths(final FileSystem fs) {
        List<String> $ = getAvailablePathsInner(fs.getReadOnlyFileSystem(FileSystemEntries.SENSORS_DATA.buildPath()),
                        new ArrayList<>());
        if ($.isEmpty())
            $.add("");
        return $;
