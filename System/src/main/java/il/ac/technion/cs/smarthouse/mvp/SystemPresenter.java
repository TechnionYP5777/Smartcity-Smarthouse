...
public abstract class SystemPresenter implements Initializable {
	...
	public final SystemCore getModel() {
		...
		systemCore.initializeSystemComponents(); // TODO: this should look
													// better...
	...
	public static class PresenterInfo {
		private final Node rootViewNode;
		private final SystemPresenter presenter;

		PresenterInfo(final Node rootViewNode, final SystemPresenter presenter) {
	...
	protected final PresenterInfo createChildPresenter(final String fxmlFileName) throws Exception {
	...
	private void securityCheck() {
		...
		final RuntimeException r = new RuntimeException(
				"Can't call " + new Throwable().getStackTrace()[1].getMethodName() + " before init");
		log.error(r.getMessage());
		throw r;
	}

	private static PresenterInfo createRootPresenter(final URL fxmlLocation) throws Exception {
		return loadPresenter(new FXMLLoader(fxmlLocation));
	}

	public static PresenterInfo createRootPresenter(final String fxmlFileName) throws Exception {
