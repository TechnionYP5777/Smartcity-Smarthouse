...
public class MessageViewController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle __) {
        ...
        this.switchModeButton.selectedProperty().addListener(new ChangeListener<Boolean>() {
    ...
    void buildStreamModePane() {
        ...
        streamMode.add(t1, 1, currentRow);
        streamMode.add(t2, 2, currentRow);
        this.timeInput=new Pair<>(t1, t2);
    }

    private boolean validateInput(String input, Types t) {
        switch (t) {
            case DOUBLE:
                try {
    ...
    public void setCurrentSensor(SensorData currentSensor) {
    ...
    void buildMessage() {
        String message = "";
        for (SensorField field : this.singleFieldMap.keySet()) {
            message += " " + field.getName() + " ";
            if (field.getType() == Types.BOOLEAN)
                message += ((ComboBox<BooleanValues>) this.singleFieldMap.get(field)).getValue();
            else {
                String Input = ((TextField) this.singleFieldMap.get(field)).getText();
