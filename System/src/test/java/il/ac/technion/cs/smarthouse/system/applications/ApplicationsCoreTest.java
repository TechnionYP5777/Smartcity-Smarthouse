...
public class ApplicationsCoreTest {
    ...
    private static String APP1_CLASSPATH = ApplicationsCoreTest.class.getPackage().getName()
                    + ".smarthouseApplicationExamples.MyApp1";
    ...
    public void addApplicationAndApplicationManagerTest() throws Exception {
        ...
        assert !appCore.getApplicationsMetaData().isEmpty();
        Assert.assertEquals(appCore.getInstalledApplicationNames().stream().filter(n -> n.equals(APP1_CLASS.getName()))
                        .count(), 1);
        ...
        ApplicationMetaData m2 = new ApplicationMetaData(m.getId(),
                        new ApplicationPath(PathType.CLASS_NAME, APP1_CLASSPATH));
        assert m.equals(m2);
        assert m2.equals(m);
    ...
    public void stupidApplicationManagerTest() {
        ApplicationMetaData m1 = new ApplicationMetaData(null,
                        new ApplicationPath(PathType.CLASS_NAME, APP1_CLASSPATH));
        ApplicationMetaData m2 = new ApplicationMetaData(null,
                        new ApplicationPath(PathType.CLASS_NAME, APP1_CLASSPATH));
        ApplicationMetaData m3 = new ApplicationMetaData("XXX",
                        new ApplicationPath(PathType.CLASS_NAME, APP1_CLASSPATH));
    ...
    public void savebleTest() throws Exception {
        ...
        Assert.assertEquals(appCoreNew.getInstalledApplicationNames().stream()
                        .filter(n -> n.equals(APP1_CLASS.getName())).count(), 1);
    }
}
